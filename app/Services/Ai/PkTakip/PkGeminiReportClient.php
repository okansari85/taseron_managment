...
/**
 * pktakip rapor algılama için Gemini istemcisi: GeminiTemplateDiscoveryClient'ın birebir kopyası.
 * Farklar: kontrol kriterleri kaldırıldı (system_criteria ve equipment_control_criteria); onların yerine
 * sisteme ve tek örnekli ekipmana genel uygunluk (verdict) eklendi.
 * Eski istemci değiştirilmemiştir.
 */
class PkGeminiReportClient
{
    ...
    private function schema(): array
    {
        ...
        // Genel uygunluk (kriterlerin yerine): status normalize sonuç, raw raporda GERÇEKTEN yazan ifade,
        // basis kararın kaynağı (explicit = raporda açıkça yazıyor, derived = kontrol sonuçlarından/bulgulardan
        // çıkarıldı, not_stated = ipucu yok -> status null), evidence kararın dayanağı olan kısa alıntı.
        $verdict = [
            'type' => 'object',
            'properties' => [
                'status' => ['type' => ['string', 'null'], 'enum' => ['uygun', 'uygun_degil', 'uygulanamiyor', null]],
                'raw' => $nullableString,
                'basis' => ['type' => 'string', 'enum' => ['explicit', 'derived', 'not_stated']],
                'evidence' => $nullableString,
            ],
            'required' => ['status', 'raw', 'basis', 'evidence'],
        ];
        ...
        $equipmentDefinition = [
            ...
            'properties' => [
                ...
                // SADECE equipment_axis="none" (tek örnek) iken gerçek değerle dolar; rows/columns iken
                // status null / basis "not_stated" kalır (örnek bazlı sonuç eskisi gibi tablodan okunur).
                'verdict' => $verdict,
            ],
            'required' => ['equipment_name', 'instance_structure', 'attributes', 'verdict'],
        ];
        ...
        $system = [
            ...
            'properties' => [
                ...
                // Sistemin genel uygunluğu (system_criteria'nın yerine).
                'verdict' => $verdict,
                'equipment_definitions' => ['type' => 'array', 'items' => $equipmentDefinition],
            ],
            'required' => ['system_name', 'section_heading_patterns', 'section_detection', 'verdict', 'equipment_definitions'],
        ];
